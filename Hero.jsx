...
function App() {
  ...
  return (
    <>
      ...
      <h1 className={`cursor-target fixed font-chomsky text-amber-950 z-40 left-1/2 -translate-x-1/2 text-4xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-8xl transition-all duration-1000 ease-out whitespace-nowrap ${
        loadingComplete 
          ? 'top-[15vh]' 
          : 'top-1/2 -translate-y-1/2'
      }`}>
        {displayedName}
        {displayedName.length < 15 && <span className="inline-block w-0.5 h-[0.9em] bg-amber-950 ml-1 animate-cursor-blink align-middle"></span>}
      </h1>
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center text-center pt-32 px-4">
      <div className="max-w-4xl mx-auto w-full">
        ...
        <p className={`text-base sm:text-lg md:text-xl text-amber-800 font-bigshot leading-relaxed mb-12 max-w-2xl mx-auto transition-all duration-700 ease-out delay-[400ms] ${
          isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-5'
        }`}>
        ...
        <div className="flex flex-wrap gap-4 justify-center mb-12">
          <button className={`cursor-target px-6 sm:px-8 py-3 bg-amber-900 text-amber-50 font-bigshot text-base sm:text-lg rounded-lg border-2 border-amber-900 hover:bg-amber-50 hover:text-amber-900 hover:border-amber-900 transition-all duration-700 shadow-lg hover:shadow-xl ${
            isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-5'
          }`}>
            View Projects
          </button>
          <button className={`cursor-target px-6 sm:px-8 py-3 bg-transparent border-2 border-amber-900 text-amber-900 font-bigshot text-base sm:text-lg rounded-lg hover:bg-amber-900 hover:text-amber-50 transition-all duration-700 ease-out ${
            isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-5'
          }`}>
            Get in Touch
          </button>
      ...
      <div className={`absolute bottom-8 right-20 transition-all duration-700 ease-out delay-[1100ms] ${
